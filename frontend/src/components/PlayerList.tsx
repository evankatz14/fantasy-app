...
export function PlayerList({ onMockAuction }: Props) {
  ...
  // After 6s of loading, the server is probably doing a cold start — tell the user
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
              ...
              <span className="text-sm">
                {slowLoad ? 'Server is warming up — this takes about 30 seconds on first load…' : 'Loading players…'}
              </span>
            </div>
          ) : (
            <RankingBoard />
